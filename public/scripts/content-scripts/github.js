wobblyButton.renderButton('#partial-discussion-sidebar', function(elem) {
    ...
    link.style = `
        ...
        `;
    link.onclick = () =>
        wobblyButton.currentTimer ? wobblyButton.timerStop() : wobblyButton.timerStart();
...
wobblyButton.renderButton('.js-project-card-details', function(elem) {
    ...
    rootElement.classList.add('wobbly');
    let container = createTag('div', 'discussion-sidebar-item js-discussion-sidebar-item');
    let link = createTag('a', 'wobbly', 'Start timer');
    link.style = `
            cursor: pointer;
            padding-left: 20px;
    ...
    if (wobblyButton.task && wobblyButton.detail && wobblyButton.project) {
        link.title = `${wobblyButton.task.textContent} / ${wobblyButton.detail.textContent} - ${wobblyButton.project.textContent}`;
    }
    container.appendChild(link);
