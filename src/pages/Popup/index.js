...
class Popup extends Component {
  ...
  componentDidMount() {
    ...
    browser.runtime.onMessage.addListener((request, sender) => {
      if (request.type === "timer-data") {
        ...
        if (request.data) {
          this.setState({ timerHistory: request.data, isFetching: false });
        }
      }
    });
    browser.storage.onChanged.addListener((changes) => {
      for (var key in changes) {
        if (
          key === "currentTimer" &&
          !changes.currentTimer.hasOwnProperty("newValue")
        ) {
  ...
  getTimerHistory = () => {
    browser.runtime.sendMessage({ type: "get-timer-history" }).then((res) => {
      if (res) {
        this.setState({ timerHistory: res.data, isFetching: false });
      }
    });
  };
  logout = () => {
  ...
  getCurrentTimerDuration = () => {
    let currentTimerDuration =
      +moment() -
      +moment(this.state.currentTimer.startDatetime) +
      this.state.currentTimer.timeDiff;
  ...
  render() {
    return (
      <div className="container">
        {!this.state.isAuth ? (
          <header className="header-login-popup">
            <img
              src={logo}
              onClick={() =>
                browser.tabs.create({ url: "https://time.wobbly.me" })
              }
            />
          </header>
        ) : (
            ...
            <div className="header-image-container">
              ...
              {/* {this.state.isFetching && (
                <div className="header-spinner">
                  <object type="image/svg+xml" data={spinnerSvg}>
                    Loading
                  </object>
                </div>
              )} */}
        ...
        {!this.state.isAuth ? (
          <div className="wrapper">
            ...
            <p className="wobbly-link">
              See more on{" "}
              <span
                onClick={() =>
                  browser.tabs.create({ url: "https://wobbly.me" })
                }
              >
                Wobbly.me
              </span>{" "}
          ...
          <div className="wrapper">
            <p className="current-timer">
              <span>Timer started:</span>
            </p>
            <div className="task-item">
              <p className="current-timer">
                {decodeURI(this.state.currentTimer.issue)} •{" "}
                {this.state.currentTimer.project.name}
              </p>
          ...
          !this.state.isFetching && (
            <div className="wrapper">
              <TimerHistoryComponent
                timerHistory={this.state.timerHistory}
