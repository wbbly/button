...
window.wobblyButton = {
  ...
  wobblyLogin: function (tabID, changeInfo, tab) {
    if (
      !wobblyButton.userAuth &&
      tab.url.indexOf("wobbly.me") > 0 &&
      changeInfo.status === "complete"
    ) {
      chrome.tabs.executeScript(tabID, {
        file: "scripts/content-scripts/wobbly.js",
      });
  ...
  tabUpdated: async function (tabID, changeInfo, tab) {
    if (
      wobblyButton.userAuth &&
      wobblyButton.checkTabForPermissions(tab.url) &&
      changeInfo.status === "complete"
    ) {
      ...
      chrome.tabs.executeScript(
        tabID,
        {
          file: `scripts/content-scripts/${wobblyButton.getExecutedScript(
            tab.url
          )}`,
        },
        () => {
          wobblyButton.contentTabs.forEach((tab) => {
            chrome.tabs.sendMessage(tab, {
              type: "timer-data",
              data: wobblyButton.currentTimer,
              projects: wobblyButton.projectList,
            });
          });
        }
      );
  ...
  getBrowserNotificationsInfo: function () {
    chrome.storage.local.get("notificationInfo", (res) => {
      if (res.notificationInfo) {
        wobblyButton.notificationInfo = wobblyButton.userAuth
    ...
    !wobblyButton.userAuth &&
      chrome.storage.local.get("unauthTimerStart", (res) => {
        if (!res.unauthTimerStart) {
          let startDelay = moment()
  ...
  getProjectList: function () {
    return new Promise((resolve) => {
      wobblyButton
        ...
        .then((result) => {
          chrome.storage.local.set({ projects: result.data });
          wobblyButton.projectList = result.data;
          resolve(true);
        });
    });
  },
  getUserHistory: function () {
    return new Promise((resolve) => {
      wobblyButton.userAuth &&
        wobblyButton
          .apiCall(`${AppConfig.apiURL}timer/user-list?page=1&limit=50`, {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
            },
          ...
          .then((result) => {
            ...
            !wobblyButton.notificationInfo.isShowed &&
              wobblyButton.notificationInfo.allowNotifications &&
              wobblyButton.startNotificationDelay(
                result.data.timer_v2[0].end_datetime || moment().format()
              );
          ...
          .then(
            (res) => {
              ...
              chrome.storage.local.set({
                notificationInfo: wobblyButton.notificationInfo,
              });
              chrome.browserAction.setIcon({
                path: "images/favicon-active.png",
              });
    ...
    wobblyButton.activeNotificationTimer = setTimeout(() => {
      if (wobblyButton.checkForAllowedTimeNotification()) {
        ...
        chrome.notifications.create(notifId, {
          message: "Don`t forgеt to start the timer",
          title: "Wobbly Button",
          type: "basic",
  ...
  startNotificationDelay: function (lastTimerStop) {
    if (wobblyButton.currentTimer || wobblyButton.activeNotificationTimer)
      return;
  ...
  editTask: function (data) {
    wobblyButton
      .apiCall(`${AppConfig.apiURL}timer/${data.taskId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
...
chrome.storage.local.get(
  ["customIntegrations", "originIntegrations"],
  (res) => {
    wobblyButton.customHostList = res.customIntegrations || [];
    wobblyButton.originHostList = res.originIntegrations || [];
    wobblyButton.origins = wobblyButton.customHostList.concat(
      wobblyButton.originHostList
    );
  }
);
...
let restartNotificationTimer = () => {
  clearTimeout(wobblyButton.activeNotificationTimer);
  wobblyButton.activeNotificationTimer = null;
  wobblyButton.showNotification(
    wobblyButton.notificationInfo.notificationDelay.value
  );
};
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "auth") {
    ...
    wobblyButton.activeNotificationTimer &&
      clearTimeout(wobblyButton.activeNotificationTimer);
    wobblyButton.activeNotificationTimer = null;
    wobblyButton.userAuth = true;
    wobblyButton.createContextMenu();
...
chrome.storage.onChanged.addListener(function (changes) {
  for (var key in changes) {
    if (
      ...
      changes.notificationInfo.hasOwnProperty("newValue") &&
      wobblyButton.userAuth
    ) {
      ...
      wobblyButton.getUserHistory();
      if (
        !changes.notificationInfo.newValue.allowNotifications &&
        wobblyButton.activeNotificationTimer
      ) {
        clearTimeout(wobblyButton.activeNotificationTimer);
        wobblyButton.activeNotificationTimer = null;
